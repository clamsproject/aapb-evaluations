# TODO (krim @ 3/23/25): replace with pathlib
import os
import shutil


# TODO (krim @ 3/23/25): previous functions are now wrapped in this class, review and update methods as needed.
class EvaluationTaskCookieCutter(object):
    ...
    def create_project_directory(self):
        ...
        project_dir = os.path.join(parent_dir, self.taskname)
        os.makedirs(project_dir, exist_ok=True)
        return project_dir

    def create_init_file(self, project_dir):
    ...
    def create_requirements_txt(self, project_dir):
    ...
    def copy_evaluation_module_template(self, project_dir):
    ...
    def copy_report_template_file(self, project_dir):
    ...
    def copy_readme_template_file(self, project_dir):
        parent_dir = os.path.dirname(os.getcwd())
        # Path to the template file in the parent directory
    ...
    def bake(self):
        project_dir = self.create_project_directory(self.taskname)
        self.create_init_file(project_dir)
        self.create_requirements_txt(project_dir)
        self.copy_evaluation_module_template(project_dir)
        self.copy_report_template_file(project_dir)
        self.copy_readme_template_file(project_dir)
...
if __name__ == "__main__":
    ...
    print()
    baker = EvaluationTaskCookieCutter(project_name)
    baker.bake()
